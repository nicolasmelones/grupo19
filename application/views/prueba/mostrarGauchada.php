...
<?php	if($emaail!=$this->session->userdata('email')){	?>
	...
	if ($resultado3 == '' ){?>
	<a href="ofrecer?id=<?php echo $row->id; ?>" onClick="return confirm('¿Está seguro que desea ofrecerse para realizar esta gauchada?');">Ofrecerse</a>
	
	
	<?php }
	else{
		echo '<b>Usted ya se ha ofrecido a realizar esta gauchada</b>';
	}
	?>
	<br>
	<br>
	<br>
	<?= form_open('/prueba/validarComentario',array('name'=>'comentario','onsubmit' => 'return validar_comentario();')) ?>
	<textarea name="comentario" rows="4" cols="50"></textarea>
	<br>
	<?=form_hidden('idG', $row->id);?>
	<?php $correo = $this->session->userdata('email'); ?>
	<?=form_hidden('idUsuarioActual', $correo);?>
	<?= form_submit('','Comentar','class="btn"') ?>
	<?= form_close() ?>	
	</center>
	<?php
	}else{
	if(($row->eliminado==0) && ($row->aceptado==0)){
		...
		?>&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp <a href="eliminarGauchada?id=<?php echo $row->id;?>&email=<?=$emaailll?>" onClick="return confirm('¿Está seguro que desea eliminar esta gauchada?                                                                              ¡Importante!                                                         Si la gauchada no tiene ofrecimientos recuperará el crédito. Caso contrario no recuperará el crédito');">Eliminar Gauchada</a><?php
		?>&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp <a href="verOfrecimientos?id=<?php echo $row->id; ?>">Ver ofrecimientos</a><?php
	}}
	...
	?><div class="d5"><br><b>Usuario:</b> <?php if ($this->session->userdata('email')){ ?><a href="verPerfil?email=<?=$eemaail?>"> <?php
	echo $eemaail;} else {echo "-";}
	...
	<b><u>Respuesta</u></b><br><br> <?php 
		...
		if($emaail==$this->session->userdata('email') && ($row2->respuesta == '')){?>
			<?= form_open('/prueba/validarRespuesta',array('name'=>'respuesta','onsubmit' => 'return validar_respuesta();')) ?>
			--------------------------------------------------------------------------------
			<textarea name="respuesta" rows="4" cols="50"></textarea><br><?php
			echo form_submit('','Responder','class="btn"');
